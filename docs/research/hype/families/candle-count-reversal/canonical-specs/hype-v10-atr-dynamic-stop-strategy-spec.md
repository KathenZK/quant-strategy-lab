# HYPE-CC-V10 ATR 动态止损策略规格

本文档用于实现 HYPE 15m V10 实盘策略。目标是让另一个 AI 或工程师只看这份文档，就能在 Rust 中实现与当前研究口径一致的 V10 版本。

## 1. 策略概要

- 策略名称：`HYPE V10 ATR 动态止损`
...
- 仓位使用 `ATR96` 动态降仓
- 止盈使用 `ATR192 * 6`，限制在 `2.0% - 4.0%`
- 止损使用 `ATR288 * 5`，限制在 `2.5% - 3.5%`
- 带 24 小时趋势禁入、反向信号间隔、平仓冷却、连续止损减半
...
V10 和 V13 的信号、趋势过滤、冷却、连续止损减半逻辑一致，区别集中在 ATR 窗口和止盈限制：

| 项目 | V10 | V13 |
|---|---:|---:|
| 动态仓位 ATR | `ATR96` | `ATR288` |
| 动态止盈 ATR | `ATR192` | `ATR288` |
| 动态止损 ATR | `ATR288` | `ATR288` |
| 止盈限制 | `2.0% - 4.0%` | `2.5% - 3.5%` |
| 止损限制 | `2.5% - 3.5%` | `2.5% - 3.5%` |
...
| `allocation_atr_window` | `96` | 仓位使用 ATR96 |
...
| `take_profit_atr_window` | `192` | 止盈使用 ATR192 |
| `take_profit_atr_multiplier` | `6.0` | 动态止盈距离等于 ATR192 百分比乘以 6 |
| `min_take_profit_pct` | `0.02` | 止盈下限 2.0% |
| `max_take_profit_pct` | `0.04` | 止盈上限 4.0% |
...
impl Default for HypeV10Config {
    fn default() -> Self {
        Self {
...
V10 同时使用 `ATR96`、`ATR192` 和 `ATR288`。三者计算方式完全一致，只是窗口不同。
...
```text
ATR_N[t] = 最近 N 根 TR 的简单移动平均，包含 t
```

ATR 百分比：

```text
atr_pct_N[t] = ATR_N[t] / close[t]
```
...
```text
atr_pct_96[t]  用于动态仓位
atr_pct_192[t] 用于动态止盈
atr_pct_288[t] 用于动态止损
```

如果任一开仓所需 ATR 为空或小于等于 0，本根 K 线不允许开仓。因为最大窗口是 288，建议实盘启动时预热至少 300 根 15m K 线。
...
base_allocation = min(
    max_allocation,
    max_allocation * target_atr_pct / atr_pct_96[t]
)
```
...
base_allocation = min(
    3.0,
    3.0 * 0.004 / atr_pct_96[t]
)
```

含义：

- 当 ATR96 小于等于 0.4% 时，仓位达到最大 3x。
- 当 ATR96 大于 0.4% 时，仓位按波动率降低。
...
```text
raw_take_profit_pct = atr_pct_192[t] * 6.0
take_profit_pct = clamp(raw_take_profit_pct, 0.02, 0.04)
```

也就是止盈百分比最低 2.0%，最高 4.0%。
...
for each closed 15m bar t:
    ...
    base_allocation = compute_atr96_allocation(t)
    entry_allocation = base_allocation * risk_multiplier

    stop_loss_pct = clamp(atr_pct_288[t] * 5.0, 0.025, 0.035)
    take_profit_pct = clamp(atr_pct_192[t] * 6.0, 0.02, 0.04)
...
8. 仓位使用 `ATR96`，不是 ATR288。
9. 止盈使用 `ATR192 * 6`，限制 `2.0% - 4.0%`。
10. 止损使用 `ATR288 * 5`，限制 `2.5% - 3.5%`。
11. ATR 是简单移动平均，不是 EMA。
12. 止盈和止损百分比在开仓时固定，持仓期间不更新。
13. 止损后下一笔仓位风险倍率减半，止盈后恢复为 1。
14. 平仓后冷却 8 根 K 线。
15. 策略没有最大持仓时间。
16. 持仓期间不根据反向信号平仓或反手。
17. 回测里同一根 K 线同时触发止盈止损时，优先算止损。
...
on_closed_bar(bar):
    ...
    atr_pct_96 = compute_atr_pct(history, 96)
    atr_pct_192 = compute_atr_pct(history, 192)
    atr_pct_288 = compute_atr_pct(history, 288)
    ...
    if atr_pct_96 <= 0 or atr_pct_192 <= 0 or atr_pct_288 <= 0:
        return

    max_allocation = 3.0
    base_allocation = min(max_allocation, max_allocation * 0.004 / atr_pct_96)
    allocation = base_allocation * risk_multiplier

    if allocation <= 0:
        return

    stop_pct = clamp(atr_pct_288 * 5.0, 0.025, 0.035)
    take_pct = clamp(atr_pct_192 * 6.0, 0.02, 0.04)
...
当前 Python 主策略代码已切换为 V13 默认值；V10 是前一版研究口径，可由同一套策略结构改参数复现。
