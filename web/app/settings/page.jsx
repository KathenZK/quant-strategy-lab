import { strategyLabApiBaseUrl } from "../../lib/strategy-lab-api";
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[420px_1fr]">
      <section className="lab-card p-4">
        ...
        <p className="mt-3 text-sm leading-6 text-zinc-600">当前仍是个人/小团队工作台，配置以本地 Strategy Lab API 和数据湖为中心。</p>
        <div className="mt-5 rounded-lg border border-zinc-200 bg-zinc-50 p-4">
          <div className="text-xs text-zinc-500">STRATEGY_LAB_API_BASE_URL</div>
          <div className="mt-2 break-all font-mono text-sm text-zinc-950">{strategyLabApiBaseUrl()}</div>
        </div>
      </section>
      ...
      <section className="lab-card p-4">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {readinessItems.map((item) => (
            <div key={item} className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm leading-6 text-zinc-600">
              {item}
            </div>
          ))}
        </div>
