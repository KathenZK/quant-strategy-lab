...
class TrendConfirmationStrategy:
    ...
    def build_signal_frame(self, factors: dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        z_momentum = _cross_section_zscore(momentum)
        z_breakout = _cross_section_zscore(breakout)
        z_oi = _cross_section_zscore(oi_change)
        z_basis = _cross_section_zscore(basis_change)
        z_volume = _cross_section_zscore(volume_surge)
        z_funding_penalty = _cross_section_zscore(funding_zscore.abs()).fillna(0.0)
    ...
    def apply_liquidation_risk_overlay(
        ...
        spike = liquidation_features.get(self.config.liquidation_spike_factor)
        ratio = liquidation_features.get(self.config.liquidation_ratio_factor)
        cooldown = liquidation_features.get(self.config.liquidation_cooldown_factor)
        ...
        for ts in adjusted.index:
            for symbol in adjusted.columns:
                ...
                if self.config.stop_on_event_cooldown and cooldown_flag > 0:
                    adjusted.loc[ts, symbol] = 0.0
                    continue

                if (
                    spike_value >= self.config.max_liquidation_spike_zscore
                    or ratio_value >= self.config.max_liquidation_notional_ratio
                ):
                    adjusted.loc[ts, symbol] = weight * self.config.liquidation_weight_scale
        return adjusted
