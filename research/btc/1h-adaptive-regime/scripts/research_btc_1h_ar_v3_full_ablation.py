...
DATE_TAG = "2026-07-06"
SUMMARY_JSON = ARTIFACT_DIR / f"btc_1h_ar_v3_full_ablation_{DATE_TAG}.json"
ROWS_CSV = ARTIFACT_DIR / f"btc_1h_ar_v3_full_ablation_rows_{DATE_TAG}.csv"
FIELDS_CSV = ARTIFACT_DIR / f"btc_1h_ar_v3_full_ablation_fields_{DATE_TAG}.csv"
REPORT_MD = ABLATION_DIR / f"btc-1h-ar-v3-full-parameter-ablation-{DATE_TAG}.md"
...
def prefit_improves(row: dict[str, Any], baseline: dict[str, Any]) -> bool:
    return bool(
        ...
        and row["prefit_win_rate"] >= 0.80
        and row["train_total_return"] > 0
        and row["validation_total_return"] > 0
        and row["train_win_rate"] >= 0.80
        and row["validation_win_rate"] >= 0.80
...
def main() -> None:
    ...
    keltner, cci = v3.v3_configs(engine)
    baseline_trades, keltner_trades, cci_trades, priorities = v3.simulate_v3(
    ...
    baseline_flat = v2_ablation.flatten_metrics(baseline_metrics)
    ...
    rows: list[dict[str, Any]] = [
        {
            "label": "BTC-1H-Adaptive-Regime-V3",
            "component": "ensemble",
            "field": "baseline",
    ...
    for component, base_cfg, base_component_trades, other_cfg in (
        ...
        values_by_field = v2_ablation.variant_values(component)
            ...
            for value in values_by_field[field_name]:
                ...
                if field_name == "style" and value == "leg_removed":
                    ...
                    if variant.min_atr_bps > variant.max_atr_bps:
                        continue
                    if component == "keltner_break":
                        merged, component_trades, _other, _priority = v3.simulate_v3(
                            engine,
                            frame,
                            funding_times,
                            funding_cumulative,
                        ...
                        merged, _other, component_trades, _priority = v3.simulate_v3(
                            engine,
                            frame,
                            funding_times,
                            funding_cumulative,
                ...
                flat = v2_ablation.flatten_metrics(v1.metrics(engine, merged))
                row = {
                    "label": label,
                    "component": component,
                    "field": field_name,
    ...
    payload = {
        ...
        "status": "full_parameter_ablation_complete_diagnostic_not_live_ready",
        "date": DATE_TAG,
    ...
    lines = [
        ...
        "",
        (
            f"已覆盖 V3 两条腿全部 `{len(field_names) * 2}` 个 StrategyConfig 字段槽，"
            f"Keltner `{len(field_names)}` 个、CCI `{len(field_names)}` 个，coverage missing 为 `0`。"
        ),
        "",
        (
            f"分类结果：`{classification_counts}`。本轮沿用 V1/V2 全消融的字段语义分类；"
            "这是 V3 冻结参数的 one-at-a-time 敏感性审计，不做组合搜索。"
        ),
        "",
        (
            f"相对 V3 基线，one-at-a-time 变体中同时满足 prefit 年化更高、回撤更小、"
            f"train/validation/prefit 胜率 >=80%、train/validation 同正且 validation DD<20% 的行数为 `{len(strict)}`。"
            "这些观察不构成 V3.1 或 promotion。"
        ),
        "",
        "## V3 基线",
        "",
