...
DATE_TAG = "2026-07-07"
SUMMARY_JSON = ARTIFACT_DIR / f"btc_1h_ar_v4_window_backtest_{DATE_TAG}.json"
WINDOWS_CSV = ARTIFACT_DIR / f"btc_1h_ar_v4_window_backtest_windows_{DATE_TAG}.csv"
TRADES_CSV = ARTIFACT_DIR / f"btc_1h_ar_v4_window_backtest_trades_{DATE_TAG}.csv"
REPORT_MD = NOTES_DIR / f"btc-1h-ar-v4-window-backtest-{DATE_TAG}.md"
...
def main() -> None:
    ...
    trades, _keltner, _cci, priorities = v4.simulate_v4(
        engine, frame, funding_times, funding_cumulative
    )
    v3_trades, *_ = v3.simulate_v3(engine, frame, funding_times, funding_cumulative)
    v3_trade_path_equal = v1.trade_signature(trades) == v1.trade_signature(v3_trades)
    if not v3_trade_path_equal:
        raise RuntimeError("V4 window backtest drifted from V3 trade path")
    ...
    payload = {
        ...
        "status": "window_backtest_registered_minimal_equivalent_not_live_ready",
        "date": DATE_TAG,
    ...
    lines = [
        ...
        "",
        (
            "V4 是按用户要求登记的 V3 最小等价参数版本：`27` 个 clean active 槽位压缩为 "
            "`19` 个必要参数，`8` 个非必要槽位以中和值固定。脚本已验证 V4 与 V3 逐笔交易"
            "签名完全一致，因此本报告的指标与 V3 多窗口回测完全相同。"
        ),
        "",
        (
            "本报告只复用 2026-07-02 已固定两年闭合 `1h` 数据，不引入新增 forward 数据。"
            "所有窗口均使用 Binance `0.001` fee/fill、`4 bps` slippage/fill 和历史资金费。"
        ),
        "",
