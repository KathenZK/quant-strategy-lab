...
DATA_PATH = ARTIFACT_DIR / "btc_binance_1h_closed_klines_2y.parquet"
QUALITY_PATH = ARTIFACT_DIR / "btc_binance_1h_data_quality_2y.json"
FUNDING_PATH = (
    ROOT
...
DATE_TAG = "2026-07-02"
SUMMARY_JSON = ARTIFACT_DIR / f"btc_1h_adaptive_regime_search_{DATE_TAG}.json"
PREFIT_CSV = ARTIFACT_DIR / f"btc_1h_adaptive_regime_prefit_{DATE_TAG}.csv"
RANKING_CSV = ARTIFACT_DIR / f"btc_1h_adaptive_regime_ranking_{DATE_TAG}.csv"
SLICES_CSV = ARTIFACT_DIR / f"btc_1h_adaptive_regime_slices_{DATE_TAG}.csv"
TRADES_CSV = ARTIFACT_DIR / f"btc_1h_adaptive_regime_top_trades_{DATE_TAG}.csv"
REPORT_MD = DIAGNOSTIC_DIR / f"btc-1h-adaptive-regime-search-{DATE_TAG}.md"
...
def load_engine() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("btc_1h_search_engine", ENGINE_PATH)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Broad locked-three-month-OOS BTCUSDT 1h adaptive-regime search."
    )
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    if not DATA_PATH.exists() or not QUALITY_PATH.exists() or not FUNDING_PATH.exists():
        raise FileNotFoundError(
            "Run scripts/fetch_btc_binance_1h.py before the search."
        )
    ...
    if blockers:
        raise RuntimeError(
            f"BTCUSDT 1h exact research frame has data-quality blockers: "
    ...
    if funding.empty or funding["funding_rate"].isna().any():
        raise RuntimeError("BTC funding history is empty or contains null rates")
...
def btc_name(name: str) -> str:
    return name.replace("HYPE_1H_AR", "BTC_1H_AR")
...
def make_report(
    ...
    *,
    ...
    lines = [
        ...
        "## 数据质量",
        "",
        f"- Binance USD-M Futures `BTCUSDT` perpetual `1h`：`{quality['rows']}` 根闭合 K。",
...
def main() -> None:
    ...
    configs = [
        replace(cfg, name=btc_name(cfg.name)) for cfg in engine.curated_configs()
    ]
    random_offset = len(configs)
    configs.extend(
        replace(
            engine.random_config(rng, index + random_offset),
            name=f"BTC_1H_AR_R{index + random_offset:06d}",
        )
    ...
    if not retained:
        raise RuntimeError("No eligible BTC configs survived the prefit gates")
