...
RUN_DATE = "2026-06-26"
REPORT_PATH = ARTIFACT_ROOT / f"hype_5m_ma_pullback_scalp_search_{RUN_DATE}.json"
SUMMARY_PATH = ARTIFACT_ROOT / f"hype_5m_ma_pullback_scalp_search_summary_{RUN_DATE}.csv"
SLICES_PATH = ARTIFACT_ROOT / f"hype_5m_ma_pullback_scalp_search_slices_{RUN_DATE}.csv"
MONTHLY_PATH = ARTIFACT_ROOT / f"hype_5m_ma_pullback_scalp_search_monthly_{RUN_DATE}.csv"
TOP_TRADES_PATH = ARTIFACT_ROOT / f"hype_5m_ma_pullback_scalp_search_top_trades_{RUN_DATE}.csv"
MARKDOWN_PATH = DIAGNOSTIC_ROOT / f"hype-5m-ma-pullback-scalp-search-{RUN_DATE}.md"
...
FEE_BPS_PER_FILL = (3.0578 / 7374.2110) * 10000.0
ENTRY_SLIPPAGE_BPS = 10.73
EXIT_SLIPPAGE_BPS = -2.64
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Search executable HYPE 5m moving-average pullback scalp variants."
    )
...
def validate_hype_5m() -> tuple[pd.DataFrame, dict[str, Any]]:
                        ...
                        for structure_window in structure_windows:
                            for tp_bps, sl_bps, hold, cooldown in exit_grid:
                                for slow_slope, fast_slope, min_atr, max_atr, rvol, adx, max_dist in filter_grid:
                                    ...
                                    configs.append(
                                        PullbackConfig(
                                            name=f"HYPE_5M_MA_PBS_C{idx:05d}",
                                            side_mode=side_mode,
...
def random_config(rng: random.Random, idx: int) -> PullbackConfig:
    ...
    return PullbackConfig(
        name=f"HYPE_5M_MA_PBS_R{idx:05d}",
...
def main() -> None:
    ...
    frame_raw, quality = validate_hype_5m()
