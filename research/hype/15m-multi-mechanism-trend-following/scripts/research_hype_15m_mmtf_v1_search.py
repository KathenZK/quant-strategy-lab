...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Search the independent HYPE 15m MMTF V1 baseline.")
...
def shortlist(frame: pd.DataFrame, per_view: int = 120) -> pd.DataFrame:
    eligible = frame.loc[
        ...
        & (frame["prefit_trades"] >= 80)
        & (frame["validation_trades"] >= 15)
...
def select_v1(frame: pd.DataFrame) -> pd.Series:
    ...
    eligible = frame.loc[
        (frame["prefit_trades"] >= 100)
        & (frame["validation_trades"] >= 20)
...
def main() -> None:
    ...
    prefix = f"hype_15m_mmtf_v1_search_{args.run_date}"
