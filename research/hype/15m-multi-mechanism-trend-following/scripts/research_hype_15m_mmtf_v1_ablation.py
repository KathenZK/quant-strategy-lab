...
def main() -> None:
    ...
    variants: list[tuple[str, engine.Config, frozenset[str], str]] = [
        ...
        (
            "inactive_entry_window_probe",
            engine.replace_config(baseline, entry_window=672),
            frozenset(),
            "Keltner 机制下探测 entry_window 是否 dormant",
        ),
        (
            "inactive_breakout_atr_probe",
            engine.replace_config(baseline, breakout_atr=0.5),
            frozenset(),
            "Keltner 机制下探测 breakout_atr 是否 dormant",
        ),
        (
    ...
    output_csv = ARTIFACT_DIR / "hype_15m_mmtf_v1_ablation_2026-07-22.csv"
    frame.to_csv(output_csv, index=False)
    summary = {
        "family": "HYPE-15M-Multi-Mechanism-Trend-Following",
        "version": "HYPE-15M-Multi-Mechanism-Trend-Following-V1",
    ...
    (ARTIFACT_DIR / "hype_15m_mmtf_v1_ablation_2026-07-22.json").write_text(
