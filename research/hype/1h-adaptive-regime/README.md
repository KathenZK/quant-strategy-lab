...
`HYPE-1H-Adaptive-Regime-V1` 已登记为历史最强冻结基线；`HYPE-1H-Adaptive-Regime-V2` 已登记为全参数消融后的干净等价版；`HYPE-1H-Adaptive-Regime-V3` 已按用户要求登记为 V2 消融引导组合 `di_roc_off__stoch_th55` 的 diagnostic baseline。三者都维持 `NO-GO / not live-ready / not promoted`。
...
V3 current full 为 `15.0530x / -19.11% / 79.73% / 74 trades`，明显强于 V2；但 reused holdout 仅 `9.0300x`，仍低于 `10x`，且前序组合复测中 K+2 current full 仅 `3.0574x / -31.93%`、8bps current full `9.4070x / -28.40%`。V3 是登记版诊断基线，不是实盘微调版本。

## 入口

- `hype-1h-ar-core-ledger.md`：本家族主账；登记 V1/V2/V3、当前状态、版本规则和后续版本约束。
