# HYPE-EMA-TB-V39.1 可复现参数说明（观察 overlay）

日期：2026-07-08

本文档登记 `HYPE-EMA-Trend-Breakout-V39.1`。V39.1 = V39 主仓 + V37 标准 early-long 小仓卫星。该组合曾短暂记录为 V40，后按用户指定降级为 V39 的小版本 / overlay 观察分支；它不是 live-ready 版本。
...
V39.1 = V39 主仓 + V37 标准 early-long 卫星
```
...
卫星量能门槛保留 V37 标准口径的 `volume_surge >= 0.25`，不跟随 V39 主仓上调到 `0.35`。测试显示 `sat_v035` 结果略差；尝试补回 V39 量能缺口的 `sat_gap` 被否决。
...
| V39.1 | +12322.33% | -24.76% | 4.91 | 149 | 73.15% | +259.76% | +2058.51% | +13621.22% |
...
- 状态：观察 overlay。
- 决策：按用户指定从 V40 降级记录为 `HYPE-EMA-TB-V39.1`。
...
- 卫星有效性诊断：`../notes/hype-ema-tb-v39-1-satellite-effectiveness-diagnostic-2026-07-08.md`
