...
完整保留 V39.3：V39.2 入场、short target 0.022、cooldown1、
TP4.8/SL6.75；只新增空单 reduce-only scale-out。
"""
...
OUT_STEM = "hype_ema_tb_v39_3_short_partial_take_profit_2026-07-17"
...
def specs() -> tuple[partial.PartialSpec, ...]:
    rows = [partial.PartialSpec("v39_3_base", None, 0.0)]
    for trigger in (4.0, 4.2, 4.4):
        for fraction in (0.50, 2.0 / 3.0, 0.75):
            rows.append(
                partial.PartialSpec(
                    f"v39_3_short_{trigger:g}_{fraction:.3f}",
                    trigger,
                    fraction,
                    "short_only",
                )
...
def main() -> None:
    ...
    if parity_diff > 1e-12:
        raise ValueError(f"V39.3 baseline parity failed: {parity_diff}")

    summary = {
        "strategy_family": "HYPE-EMA-Trend-Breakout",
        "registered_reference": "HYPE-EMA-TB-V39.3",
        "audit_id": "V39.3 short-only one-stage partial take-profit",
        "run_date": "2026-07-17",
        "status": "diagnostic_only_v39_3_unchanged",
        ...
        "assumptions": {
            "partial_fill": (
                ...
                "target; remaining allocation keeps V39.3 TP4.8/SL6.75."
            ),
            "same_bar_order": (
                "Stop-first; without a stop hit, partial target fills before "
                "TP4.8 when both are touched in one bar."
            ),
            "path": (
                "Partial fill does not close the strategy position; V39.3 "
                "cooldown1 starts only after the final exit."
            ),
            "unchanged": (
                "V39.3 long_vol_min=0.25, short target 0.022, no short 1h "
                "EMA confirm, K0/K1/K2 timing, ADX22 delayed3, MFE1.5 "
                "indicator-exit disable, timeout384 and cooldown1."
            ),
            "cost": (
        ...
        "runs": [
            {
                ...
                "comparison_to_v39_3": v39_partial.compare_to_base(
                    run,
                    baseline,
                ),
            }
