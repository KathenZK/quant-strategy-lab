"""V35 在 MFE>=1.5 后按当前 ATR 重设 entry-anchored TP/SL。

重设只使用已收盘 15m K 的 ATR，并从下一根 K 生效。该脚本比较一次冻结、
逐根更新，以及是否允许波动上升时把原始风险继续放大。
"""

from __future__ import annotations

import json
from dataclasses import asdict, dataclass
from pathlib import Path
from typing import Any

import numpy as np
import pandas as pd

import research_hype_ema_tb_v35_full_ablation_recent_tune as signal_engine
import research_hype_ema_tb_v35_h4_rsi6_entry_filter as data_diag
import research_hype_ema_tb_v35_profit_floor as base


ROOT = Path("research/hype/15m-ema-trend-breakout")
ARTIFACT_DIR = ROOT / "artifacts"
OUT_STEM = "hype_ema_tb_v35_mfe15_current_atr_bracket_2026-07-17"
SUMMARY_PATH = ARTIFACT_DIR / f"{OUT_STEM}.json"
TRADES_PATH = ARTIFACT_DIR / f"{OUT_STEM}_trades.csv"
EQUITY_PATH = ARTIFACT_DIR / f"{OUT_STEM}_equity.csv"


@dataclass(frozen=True, slots=True)
class ResetSpec:
    name: str
    atr_window: int
    update_mode: str
    allow_widen: bool
    anchor_mode: str = "entry"


def specs(config: base.V35Config) -> tuple[ResetSpec, ...]:
    return (
        ResetSpec("v35_base", config.atr_window, "disabled", False),
        ResetSpec("freeze_atr672_raw", 672, "freeze", True),
        ResetSpec("freeze_atr672_shrink_only", 672, "freeze", False),
        ResetSpec("rolling_atr672_raw", 672, "rolling", True),
        ResetSpec("rolling_atr672_shrink_ratchet", 672, "rolling", False),
        ResetSpec("freeze_atr336_shrink_only", 336, "freeze", False),
        ResetSpec("freeze_atr192_shrink_only", 192, "freeze", False),
        ResetSpec("freeze_atr96_shrink_only", 96, "freeze", False),
        ResetSpec(
            "freeze_atr672_current_close_raw",
            672,
            "freeze",
            True,
            "current_close",
        ),
        ResetSpec(
            "freeze_atr672_current_close_shrink_only",
            672,
            "freeze",
            False,
            "current_close",
        ),
        ResetSpec(
            "freeze_atr192_current_close_shrink_only",
            192,
            "freeze",
            False,
            "current_close",
        ),
    )


def bracket_exit(
    *,
    position: base.Position,
    open_price: float,
    high: float,
    low: float,
    config: base.V35Config,
    active_atr: float,
    anchor_price: float,
) -> tuple[str, float] | None:
    take = (
        anchor_price
        + position.direction * config.take_profit_atr * active_atr
    )
    stop = (
        anchor_price
        - position.direction * config.hard_stop_atr * active_atr
    )
    if position.direction == 1:
        if low <= stop:
            return "stop_loss", min(open_price, stop) if open_price <= stop else stop
        if high >= take:
            return "take_profit", take
    else:
        if high >= stop:
            return "stop_loss", max(open_price, stop) if open_price >= stop else stop
        if low <= take:
            return "take_profit", take
    return None


def close_position(
    *,
    equity: float,
    position: base.Position,
    exit_price: float,
    exit_ts: pd.Timestamp,
    exit_bar: int,
    reason: str,
    trades: list[dict[str, Any]],
    config: base.V35Config,
    active_atr: float,
    reset_count: int,
    reset_ts: pd.Timestamp | None,
    first_reset_atr: float | None,
    anchor_price: float,
) -> tuple[float, float]:
    pnl = position.direction * position.allocation * (
        exit_price / position.previous_price - 1.0
    )
    cost = config.trade_cost_rate * position.allocation
    exit_equity = equity * (1.0 + pnl - cost)
    trades.append(
        {
            "entry_ts": position.entry_ts,
            "exit_ts": exit_ts,
            "direction": position.direction,
            "entry_price": position.entry_price,
            "exit_price": exit_price,
            "entry_atr": position.entry_atr,
            "active_atr_at_exit": active_atr,
            "active_to_entry_atr_ratio": active_atr / position.entry_atr,
            "bracket_anchor_price_at_exit": anchor_price,
            "allocation": position.allocation,
            "mfe_atr": position.mfe_atr,
            "exit_reason": reason,
            "entry_bar": position.entry_bar,
            "exit_bar": exit_bar,
            "hold_bars": exit_bar - position.entry_bar,
            "raw_price_return": position.direction
            * (exit_price / position.entry_price - 1.0),
            "trade_return": exit_equity / position.entry_equity - 1.0,
            "entry_equity": position.entry_equity,
            "exit_equity": exit_equity,
            "reset_count": reset_count,
            "first_reset_ts": reset_ts,
            "first_reset_atr": first_reset_atr,
            "first_reset_to_entry_ratio": (
                None
                if first_reset_atr is None
                else first_reset_atr / position.entry_atr
            ),
        }
    )
    return exit_equity, cost


def reset_basis(
    *,
    spec: ResetSpec,
    entry_atr: float,
    active_atr: float,
    current_atr: float,
) -> float:
    if spec.allow_widen:
        return current_atr
    return min(entry_atr, active_atr, current_atr)


def run_backtest(
    *,
    spec: ResetSpec,
    frame: pd.DataFrame,
    funding: pd.Series,
    features: pd.DataFrame,
    config: base.V35Config,
) -> tuple[base.RunResult, dict[str, Any]]:
    start = max(config.warmup_bars, config.entry_delay_bars + 1)
    equity = 1.0
    position: base.Position | None = None
    active_atr = np.nan
    anchor_price = np.nan
    reset_count = 0
    first_reset_ts: pd.Timestamp | None = None
    first_reset_atr: float | None = None
    pending_exit: str | None = None
    last_exit_bar = -1
    equity_values: list[float] = []
    period_returns: list[float] = []
    weight_values: list[float] = []
    trades: list[dict[str, Any]] = []
    trading_costs = 0.0
    funding_pnl_total = 0.0
    reset_events = 0
    reset_ratios: list[float] = []

    for i in range(start, len(frame)):
        start_equity = equity
        ts = pd.Timestamp(frame.index[i])
        open_price = float(frame["open"].iloc[i])
        high = float(frame["high"].iloc[i])
        low = float(frame["low"].iloc[i])
        close = float(frame["close"].iloc[i])
        exited_this_bar = False

        if position is not None and pending_exit is not None:
            equity, cost = close_position(
                equity=equity,
                position=position,
                exit_price=open_price,
                exit_ts=ts,
                exit_bar=i,
                reason=pending_exit,
                trades=trades,
                config=config,
                active_atr=active_atr,
                reset_count=reset_count,
                reset_ts=first_reset_ts,
                first_reset_atr=first_reset_atr,
                anchor_price=anchor_price,
            )
            trading_costs += cost
            position = None
            pending_exit = None
            last_exit_bar = i
            exited_this_bar = True

        if position is not None:
            funding_pnl = (
                -position.direction
                * position.allocation
                * float(funding.iloc[i])
            )
            equity *= 1.0 + funding_pnl
            funding_pnl_total += funding_pnl

        if position is None and not exited_this_bar and i > last_exit_bar:
            signal_i = i - config.entry_delay_bars
            direction = 0
            if bool(features["long_signal"].iloc[signal_i]) and not bool(
                features["short_signal"].iloc[signal_i]
            ):
                direction = 1
            elif bool(features["short_signal"].iloc[signal_i]) and not bool(
                features["long_signal"].iloc[signal_i]
            ):
                direction = -1
            entry_atr = float(features["atr"].iloc[i - 1])
            if (
                direction != 0
                and np.isfinite(entry_atr)
                and entry_atr > 0.0
                and open_price > 0.0
            ):
                target = (
                    config.long_target_atr_pct
                    if direction == 1
                    else config.short_target_atr_pct
                )
                allocation = min(
                    config.max_allocation,
                    target / (entry_atr / open_price),
                )
                cost = config.trade_cost_rate * allocation
                equity *= 1.0 - cost
                trading_costs += cost
                position = base.Position(
                    direction=direction,
                    entry_bar=i,
                    entry_ts=ts,
                    entry_price=open_price,
                    entry_atr=entry_atr,
                    allocation=allocation,
                    entry_equity=equity,
                    previous_price=open_price,
                )
                active_atr = entry_atr
                anchor_price = open_price
                reset_count = 0
                first_reset_ts = None
                first_reset_atr = None

        if position is not None:
            intrabar = bracket_exit(
                position=position,
                open_price=open_price,
                high=high,
                low=low,
                config=config,
                active_atr=active_atr,
                anchor_price=anchor_price,
            )
            if intrabar is not None:
                reason, exit_price = intrabar
                equity, cost = close_position(
                    equity=equity,
                    position=position,
                    exit_price=exit_price,
                    exit_ts=ts,
                    exit_bar=i,
                    reason=reason,
                    trades=trades,
                    config=config,
                    active_atr=active_atr,
                    reset_count=reset_count,
                    reset_ts=first_reset_ts,
                    first_reset_atr=first_reset_atr,
                    anchor_price=anchor_price,
                )
                trading_costs += cost
                position = None
                pending_exit = None
                last_exit_bar = i
            else:
                pnl = position.direction * position.allocation * (
                    close / position.previous_price - 1.0
                )
                equity *= 1.0 + pnl
                position.previous_price = close
                base.update_position_on_close(
                    position,
                    high,
                    low,
                    config,
                    base.ProfitFloorConfig(enabled=False),
                )
                can_indicator_exit = (
                    position.mfe_atr < config.disable_after_mfe_atr
                )
                if (
                    can_indicator_exit
                    and float(features["adx"].iloc[i]) < config.adx_exit
                ):
                    position.weak_bars += 1
                else:
                    position.weak_bars = 0
                if (
                    can_indicator_exit
                    and position.weak_bars >= config.delayed_bars
                ):
                    pending_exit = "indicator_exit"
                if (
                    pending_exit is None
                    and i - position.entry_bar >= config.max_hold_bars
                ):
                    pending_exit = "timeout"

                should_reset = (
                    spec.update_mode != "disabled"
                    and position.mfe_atr
                    >= config.disable_after_mfe_atr
                    and (
                        spec.update_mode == "rolling"
                        or reset_count == 0
                    )
                )
                current_atr = float(
                    features[f"atr_{spec.atr_window}"].iloc[i]
                )
                if (
                    should_reset
                    and np.isfinite(current_atr)
                    and current_atr > 0.0
                ):
                    next_atr = reset_basis(
                        spec=spec,
                        entry_atr=position.entry_atr,
                        active_atr=active_atr,
                        current_atr=current_atr,
                    )
                    if reset_count == 0:
                        first_reset_ts = ts
                        first_reset_atr = next_atr
                        reset_ratios.append(
                            next_atr / position.entry_atr
                        )
                        reset_events += 1
                    active_atr = next_atr
                    if spec.anchor_mode == "current_close":
                        anchor_price = close
                    reset_count += 1

        equity_values.append(equity)
        period_returns.append(equity / start_equity - 1.0)
        weight_values.append(
            0.0
            if position is None
            else position.direction * position.allocation
        )

    index = frame.index[start:]
    equity_curve = pd.Series(equity_values, index=index, name=spec.name)
    returns = pd.Series(
        period_returns,
        index=index,
        name=f"{spec.name}_return",
    )
    weights = pd.Series(
        weight_values,
        index=index,
        name=f"{spec.name}_weight",
    )
    trades_frame = pd.DataFrame(trades)
    metrics = base.metrics_from_series(
        equity_curve=equity_curve,
        returns=returns,
        weights=weights,
        trades=trades_frame,
        trading_costs=trading_costs,
        funding_pnl=funding_pnl_total,
    )
    metrics["reset_events"] = reset_events
    open_position = None
    if position is not None:
        open_position = base.open_position_summary(
            position,
            frame.index[-1],
        )
        open_position.update(
            {
                "active_atr": active_atr,
                "bracket_anchor_price": anchor_price,
                "active_to_entry_atr_ratio": (
                    active_atr / position.entry_atr
                ),
                "reset_count": reset_count,
                "first_reset_ts": first_reset_ts,
                "first_reset_atr": first_reset_atr,
            }
        )
    run = base.RunResult(
        name=spec.name,
        metrics=metrics,
        slices=base.slice_metrics(equity_curve, trades_frame),
        trades=trades_frame,
        equity_curve=equity_curve,
        period_returns=returns,
        open_position=open_position,
    )
    audit = {
        "reset_events": reset_events,
        "first_reset_ratio": {
            "min": (
                None if not reset_ratios else min(reset_ratios)
            ),
            "median": (
                None
                if not reset_ratios
                else float(np.median(reset_ratios))
            ),
            "max": (
                None if not reset_ratios else max(reset_ratios)
            ),
        },
        "reset_trade_exit_counts": (
            trades_frame.loc[
                trades_frame.get(
                    "reset_count",
                    pd.Series(0, index=trades_frame.index),
                ).gt(0),
                "exit_reason",
            ]
            .value_counts()
            .to_dict()
        ),
    }
    return run, audit


def comparison(
    run: base.RunResult,
    baseline: base.RunResult,
) -> dict[str, Any] | None:
    if run is baseline:
        return None
    return {
        "final_equity_retained_pct": round(
            100.0
            * (1.0 + run.metrics["return_pct"] / 100.0)
            / (1.0 + baseline.metrics["return_pct"] / 100.0),
            2,
        ),
        "return_delta_pp": round(
            run.metrics["return_pct"]
            - baseline.metrics["return_pct"],
            2,
        ),
        "max_drawdown_delta_pp": round(
            run.metrics["max_drawdown_pct"]
            - baseline.metrics["max_drawdown_pct"],
            2,
        ),
        "sharpe_delta": round(
            run.metrics["sharpe"] - baseline.metrics["sharpe"],
            2,
        ),
        "trade_delta": (
            run.metrics["trades"] - baseline.metrics["trades"]
        ),
        "win_rate_delta_pp": round(
            run.metrics["win_rate_pct"]
            - baseline.metrics["win_rate_pct"],
            2,
        ),
    }


def main() -> None:
    ARTIFACT_DIR.mkdir(parents=True, exist_ok=True)
    warehouse = base.DuckDBWarehouse(
        base.DataLakeLayout.from_settings(base.load_settings(None))
    )
    frame, funding, quality = data_diag.load_data(warehouse)
    quality_gate = data_diag.quality_gate(quality)
    config = base.V35Config()
    flags = signal_engine.SignalFlags()
    features = signal_engine.build_signals(
        base.build_features(frame, config),
        config,
        flags,
    )
    tr = base.true_range(frame)
    run_specs = specs(config)
    for window in sorted({spec.atr_window for spec in run_specs}):
        features[f"atr_{window}"] = tr.rolling(
            window,
            min_periods=window,
        ).mean()

    outputs = [
        (
            spec,
            *run_backtest(
                spec=spec,
                frame=frame,
                funding=funding,
                features=features,
                config=config,
            ),
        )
        for spec in run_specs
    ]
    baseline = outputs[0][1]
    canonical = base.run_backtest(
        "v35_canonical",
        frame,
        funding,
        features,
        config,
        base.ProfitFloorConfig(enabled=False),
    )
    parity_diff = float(
        (canonical.equity_curve - baseline.equity_curve).abs().max()
    )
    if parity_diff > 1e-12:
        raise ValueError(f"V35 baseline parity failed: {parity_diff}")

    summary = {
        "strategy_family": "HYPE-EMA-Trend-Breakout",
        "registered_reference": "HYPE-EMA-TB-V35",
        "audit_id": "V35 MFE1.5 current-ATR bracket reset",
        "run_date": "2026-07-17",
        "status": "diagnostic_only_v35_unchanged",
        "data_quality": quality,
        "gates": {
            "data_quality": quality_gate,
            "canonical_vs_custom_baseline_max_equity_diff": parity_diff,
        },
        "assumptions": {
            "trigger": (
                "MFE>=1.5 entry ATR observed on a closed 15m bar."
            ),
            "timing": (
                "Current ATR is computed through that close; the reset "
                "TP5/SL7 bracket is active next bar."
            ),
            "anchor": (
                "Entry variants retain the entry-price anchor; current_close "
                "variants re-anchor once at the trigger bar close."
            ),
            "freeze": "Reset once at the first eligible close.",
            "rolling": (
                "Recompute after every eligible close for the next bar."
            ),
            "shrink_only": (
                "ATR basis may not exceed entry ATR and may never increase "
                "after a prior shrink."
            ),
            "same_bar": (
                "Stop-first; stop gaps fill at the worse open while TP fills "
                "at the fixed limit."
            ),
            "cost": "0.00085 per entry/exit fill plus Binance funding.",
        },
        "config": asdict(config),
        "signal_flags": asdict(flags),
        "runs": [
            {
                "spec": asdict(spec),
                "metrics": run.metrics,
                "standard_slices": run.slices,
                "open_position": run.open_position,
                "audit": audit,
                "comparison_to_v35": comparison(run, baseline),
            }
            for spec, run, audit in outputs
        ],
    }
    SUMMARY_PATH.write_text(
        json.dumps(summary, indent=2, ensure_ascii=False, default=str),
        encoding="utf-8",
    )
    pd.concat(
        [
            run.trades.assign(variant=run.name)
            for _, run, _ in outputs
        ],
        ignore_index=True,
    ).to_csv(TRADES_PATH, index=False)
    pd.concat(
        [run.equity_curve.rename(run.name) for _, run, _ in outputs],
        axis=1,
    ).to_csv(EQUITY_PATH, index_label="ts")

    print(
        f"data: {quality['start']} ~ {quality['end']} rows={quality['rows']} "
        f"quality_gate={quality_gate['passed']}"
    )
    print(f"baseline parity diff={parity_diff:.2e}")
    print(
        f"{'variant':>36} {'return%':>10} {'maxDD%':>8} "
        f"{'sharpe':>7} {'trades':>6} {'win%':>7} {'resets':>7}"
    )
    for _, run, audit in outputs:
        metrics = run.metrics
        print(
            f"{run.name:>36} {metrics['return_pct']:>10.2f} "
            f"{metrics['max_drawdown_pct']:>8.2f} "
            f"{metrics['sharpe']:>7.2f} {metrics['trades']:>6} "
            f"{metrics['win_rate_pct']:>7.2f} "
            f"{audit['reset_events']:>7}"
        )
    print(f"summary -> {SUMMARY_PATH}")


if __name__ == "__main__":
    main()
