...
OUT_STEM = "hype_ema_tb_v35_3_long_adx_threshold_scan_2026-07-20"
...
THRESHOLDS = (24.0, 25.0, 26.0, 27.0, 28.0, 29.0, 30.0)
...
def main() -> None:
    ...
    for threshold in THRESHOLDS:
        config = replace(base_config, long_adx_min=threshold)
    ...
    baseline = next(run for threshold, _, run, _ in outputs if threshold == 28.0)
    summary = {
        "strategy_family": "HYPE-EMA-Trend-Breakout",
        "registered_reference": "HYPE-EMA-TB-V35.3",
        "audit_id": "V35.3 long 15m ADX28 threshold scan",
        "run_date": "2026-07-20",
        "status": "diagnostic_only_v35_3_unchanged",
        "data_quality": quality,
        "gates": {"data_quality": quality_gate},
        "assumptions": {
            "only_change": "Long 15m ADX28 entry threshold.",
            "thresholds": list(THRESHOLDS),
            "unchanged": (
                "V35.3 1h filters, sizing, long SL6.75, short SL5.7, "
                "short MFE4.4 reduce 75%, and all other entry/exit rules."
            ),
            "costs": (
    ...
    pd.concat(
        [
            run.trades.assign(long_adx_threshold=threshold)
    ...
    print(
        ...
        f"{'sharpe':>7} {'trades':>6} {'win%':>7} {'longs':>6}"
    )
    for threshold, _, run, _ in outputs:
        metrics = run.metrics
        print(
            ...
            f"{metrics['win_rate_pct']:>7.2f} {metrics['long_trades']:>6}"
        )
