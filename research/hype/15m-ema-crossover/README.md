...
- 当前状态：`HYPE-EMA-X-V18` 为 `dry-run / forward-test required`（quant-runner `hype_ema_x` 模拟盘运行中，runner 观察报告见 [runner-tracking/README.md](runner-tracking/README.md)）；V15-V17.1 保留为 `registered / not promoted / not live-ready`。
...
- V18 forward 跟踪：[runner-tracking/README.md](runner-tracking/README.md)

按需打开 `diagnostics/`（执行审计、可行性复审）、`ablations/`、`notes/`（历史规则镜像）；脚本在 `scripts/`，证据在 `artifacts/`，`legacy-canvas/` 为冻结迁移历史。
