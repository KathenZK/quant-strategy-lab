...
`V1/V1.1/V1.2/V1.3 registered paper-audit observations / NO-GO for original 3-5 trades/day shape / not promoted / not live-ready`。

原始目标形态（`3-5` 笔/天高胜率 micro-profit）在可用样本、executable order model 和观测 Binance 成本下不可行；放宽频率至约 `0.3-0.5` 笔/天后才出现正期望候选。所有版本最多推进到逐笔 paper audit，不得标记 candidate、dry-run、handoff 或 live。
