...
- `HYPE-5M-Micro-Scalp-V1.2`：由 `V1.1_tune_grid_004895` 正式登记而来；相对 V1.1 调整 EMA HTF、ADX/Chop/RVOL/ATR 过滤及 TP/SL。指定成本下收益更高但 maxDD 略深，默认 `1x`，仍为 paper-audit observation / not live-ready。
- `HYPE-5M-Micro-Scalp-V1.3`：V1.2 的精简 schema 登记版；剔除 dormant 与等效关闭字段，交易逻辑与 V1.2 逐笔等价。当前 canonical 配置表只暴露 `18` 个有效参数；仍为 paper-audit observation / not live-ready。
