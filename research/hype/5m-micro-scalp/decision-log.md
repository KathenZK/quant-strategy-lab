...
- `HYPE-5M-Micro-Scalp-V1.1`：由 `V1S_rand_016782__N00596` 记录而来，是当前 paper-audit observation baseline；它改善了收益并使 maxDD 略浅于当前数据 V1，但仍需逐笔路径图、同 K TP/SL 与 gap ordering、参数邻域二次收缩、walk-forward 固化、订单维护和 restart-state 审计，不是 live/paper-live/handoff。
- `V1.1_tune_grid_004895`：当前 V1.1 微调搜索中的优先后续观察行，收益更高且回撤略浅于 V1.1，但还没有二次邻域、逐笔路径、walk-forward 或 live-executable 审计，不能作为正式升级版本。
