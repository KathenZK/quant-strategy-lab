...
ENTRY_WINDOWS = (12, 18, 24, 36, 48, 72, 96, 120, 168)
EXIT_WINDOWS = (6, 9, 12, 18, 24, 36, 48, 72)
EMA_SPANS = (8, 12, 16, 24, 32, 48, 72, 96, 120, 168, 240, 384)
ATR_WINDOWS = (10, 14, 20, 28, 48)
...
def _funding_by_bar(ts: pd.DatetimeIndex, funding: pd.DataFrame) -> np.ndarray:
    ...
    closes = (ts + pd.Timedelta(hours=1)).as_unit("ns").asi8
...
def build_book(*, include_locked_oos: bool = False) -> FeatureBook:
    ...
    rvol = volume / (
        pd.Series(volume).shift(1).rolling(48, min_periods=48).median().to_numpy("float64")
    )
    return FeatureBook(
        ts=ts,
