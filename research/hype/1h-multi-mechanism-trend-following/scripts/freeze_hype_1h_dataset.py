...
SOURCE_METADATA = (
    ROOT
    / "research/hype/1h-adaptive-regime/artifacts"
    / "hype_binance_1h_data_quality.json"
)
...
def main() -> None:
    ...
    terminal = pd.Timestamp(normalized["ts"].iloc[-1]) + pd.Timedelta(hours=1)
    ...
    if prefit.empty or locked.empty:
        raise RuntimeError("prefit or locked OOS partition is empty")
    ...
    manifest = {
        ...
        "freeze_contract": {
            ...
            "reveal_rule": "locked OOS may be evaluated once only after the final candidate and code hash are frozen",
        },
        "rows": {
        ...
        "funding_quality": {
            ...
            "max_gap_hours": float(funding["ts"].diff().dropna().max().total_seconds() / 3600.0),
        },
        "hashes": {
    ...
    output = ARTIFACT_DIR / "hype_1h_mmtf_dataset_freeze_2026-07-22.json"
