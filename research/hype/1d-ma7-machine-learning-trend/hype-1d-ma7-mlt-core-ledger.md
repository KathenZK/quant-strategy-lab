...
- 当前实验：P7 供体合并 OOF AUC `0.617` 通过 `>=0.60`，但 HYPE 最后80日 survival-only 覆盖与 V7.1 完全重合（`+21.70%/-14.95%/2 trades`），未严格提高收益；完整365日迁移 `+398.61%` 弱于 V7.1 `+515.73%`。开发门禁失败，状态 `DEVELOPMENT_FAILED_HOLDOUT_LOCKED`。
- 下一门禁：P7 禁止读取后81日，现有 manifest 已冻结 `holdout_permitted=false`。后续实验必须另立合同，只能依据前365日与供体窗口设计；不得为通过门禁而降低阈值、把 HYPE 放入训练集或读取后81日。P0–P7 均不授权 runner。
