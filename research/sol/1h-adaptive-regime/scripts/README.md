...
- `research_sol_1h_ar_v1_full_ablation.py`：登记 V1 后覆盖每条腿全部配置字段，输出路径等价、严格改善和 clean-surface 分类。
- `sol_1h_ar_v1_clean.py`：从消融 JSON 动态构建 clean 配置类型，把非 active 字段从调参接口移除，并要求 V1 逐笔签名完全相等。
- `research_sol_1h_ar_v1_clean_tune.py`：基于消融保留字段做高密度微调；选择不使用 reused OOS，胜率只要求适中且评分在 `65%` 封顶。
