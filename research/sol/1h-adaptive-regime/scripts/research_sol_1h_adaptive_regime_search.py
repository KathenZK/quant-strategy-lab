...
DATE_TAG = "2026-07-03"
SUMMARY_JSON = ARTIFACT_DIR / f"sol_1h_adaptive_regime_search_{DATE_TAG}.json"
PREFIT_CSV = ARTIFACT_DIR / f"sol_1h_adaptive_regime_prefit_{DATE_TAG}.csv"
RANKING_CSV = ARTIFACT_DIR / f"sol_1h_adaptive_regime_ranking_{DATE_TAG}.csv"
SLICES_CSV = ARTIFACT_DIR / f"sol_1h_adaptive_regime_slices_{DATE_TAG}.csv"
TRADES_CSV = ARTIFACT_DIR / f"sol_1h_adaptive_regime_top_trades_{DATE_TAG}.csv"
REPORT_MD = DIAGNOSTIC_DIR / f"sol-1h-adaptive-regime-search-{DATE_TAG}.md"
...
def load_engine() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("sol_1h_search_engine", ENGINE_PATH)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Broad locked-three-month-OOS SOLUSDT 1h adaptive-regime search."
    )
...
def _load_funding() -> pd.DataFrame:
    if not FUNDING_PATH.exists():
        raise FileNotFoundError(
            "SOL funding history is missing from the normalized data lake."
        )
    ...
    if funding.empty or funding["funding_rate"].isna().any():
        raise RuntimeError("SOL funding history is empty or contains null rates")
    return funding
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    if blockers:
        raise RuntimeError(
            f"SOLUSDT 1h exact research frame has data-quality blockers: "
...
def sol_name(name: str) -> str:
    return name.replace("HYPE_1H_AR", "SOL_1H_AR")
...
def make_report(
    ...
    *,
    ...
    lines = [
        ...
        "## 数据质量",
        "",
        f"- Binance USD-M Futures `SOLUSDT` perpetual `1h`：`{quality['rows']}` 根闭合 K。",
...
def main() -> None:
    ...
    configs = [
        replace(cfg, name=sol_name(cfg.name)) for cfg in engine.curated_configs()
    ]
    random_offset = len(configs)
    configs.extend(
        replace(
            engine.random_config(rng, index + random_offset),
            name=f"SOL_1H_AR_R{index + random_offset:06d}",
        )
