...
主状态只有下面这些；研究侧 3 个，promotion 侧 3 个，外加 dry-run/live 后的终态 NO-GO：
...
audit
   ...
   ▼
live spec
   │ quant-runner 实现完成 + 指标对拍/smoke test 通过 + forward-tracking 建立
   ▼
...
`live spec`、`dry-run`、`live` 以及移交语义的 `handoff` 属于 promotion 状态；进入任何 promotion 状态前必须完成 live-executable 审计（见 `.cursor/rules/live-executable-strategy-research.mdc` 与 `.cursor/rules/lab-runner-handoff.mdc`）。本仓库不定义额外的模拟盘阶段；模拟盘/仿真运行统一称为 `dry-run`，真实小额下单归入 `live`。
...
| `dry-run` | 在 quant-runner 以 dry-run 模式运行（模拟盘，不下真实订单） | quant-runner 实现完成；指标对拍/smoke test 通过；`forward-tracking/` 建立 |
...
`handoff` / "交接版本"：把规格与实现移交给人或其他系统维护的动作标签，可叠加在 `live spec` 及之后的主状态上；要求双向链接的 SPEC 齐备、参数一致性验证记录在案。
...
- 状态词必须与完整 family name + 版本号一起出现，例如 `HYPE-15M-MII-V1.3：live spec / not live-ready`。
