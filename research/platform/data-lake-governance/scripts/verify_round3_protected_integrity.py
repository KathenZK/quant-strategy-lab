#!/usr/bin/env python3
"""Verify round-3 protected lake and frozen evidence were not mutated."""
...
DEFAULT_SNAPSHOT = (
    ...
    / "pre_round3_protected_inventory_2026-09-03.csv"
)
CHECK_PREFIXES = (
    "data/raw/",
...
def main() -> None:
    ...
    for relpath, row in expected.items():
        ...
        if str(int(stat.st_size)) != str(row["size"]) or digest != row["sha256"]:
            changed.append(relpath)
    if missing or changed:
        raise SystemExit(
            f"round-3 protected files changed: missing={len(missing)} changed={len(changed)}; "
            f"examples={(missing + changed)[:8]}"
        )
    print(f"ok: {len(expected)} round-3 protected files unchanged")
