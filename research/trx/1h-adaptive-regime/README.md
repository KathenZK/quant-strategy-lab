...
- 当前状态：V1-V3 已登记；V3 clean 面微调与近期适配复搜均无命中，V3 为局部最优；`NO-GO / not promoted / not live-ready`。

## 研究协议（冻结口径）

- 数据：最近两年全部闭合 `1h` K 刷新自 Binance FAPI，raw/normalized 数据湖分区 + 资金费历史 + 合约过滤器快照。
- OOS：最后三个月 locked out-of-sample，搜索、排序、微调和 ensemble 冻结不得读取。
...
- 实盘可行性 NO-GO 证据：`live-specs/trx-1h-ar-live-feasibility-2026-07-03.md`
