...
def load_engine() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("trx_1h_fetch_engine", ENGINE_PATH)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and audit the latest two years of closed Binance TRXUSDT "
            "perpetual 1h bars."
        )
    )
...
def fetch_two_year_klines(
    ...
    while cursor < end_ms:
        ...
        time.sleep(0.05)
    if not rows:
        raise RuntimeError("Binance returned no TRXUSDT 1h klines")
    frame = pd.DataFrame(
        rows,
        columns=[
            "open_time",
...
def main() -> None:
    ...
    raw = fetch_two_year_klines(
        engine, timeout=args.timeout, cutoff_ms=server_ms
    )
    ...
    research_frame_path = (
        ARTIFACT_DIR / "trx_binance_1h_closed_klines_2y.parquet"
    )
    normalized.to_parquet(research_frame_path, index=False)
    funding.to_csv(
        ARTIFACT_DIR / "trx_binance_funding_history_2y.csv", index=False
    )
    contract_path = (
        ARTIFACT_DIR / "trx_binance_contract_snapshot_2026-07-03.json"
    )
    ...
    metadata = {
        ...
        "paths": {
            ...
            "contract_snapshot": str(contract_path.relative_to(ROOT)),
        },
        "funding": {
    ...
    output = ARTIFACT_DIR / "trx_binance_1h_data_quality_2y.json"
    output.write_text(
