...
复现顺序：fetch → broad search → refine → persistent boundary → live feasibility → V1base full ablation → V2 strict ablation/slices → recent adaptation search。所有搜索脚本的默认 seed、搜索规模和输出路径均已冻结在源码中。
