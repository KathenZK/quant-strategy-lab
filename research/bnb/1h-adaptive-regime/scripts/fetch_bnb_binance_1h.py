...
ENGINE_PATH = (
    ROOT / "research/hype/1h-adaptive-regime/scripts/fetch_hype_binance_1h.py"
)
...
def load_engine() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("bnb_1h_fetch_engine", ENGINE_PATH)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch and audit the latest two years of closed Binance BNBUSDT "
            "perpetual 1h bars."
        )
    )
...
def fetch_two_year_klines(
    engine: Any, *, timeout: float, cutoff_ms: int
) -> pd.DataFrame:
    ...
    while cursor < end_ms:
        ...
        time.sleep(0.05)
    if not rows:
        raise RuntimeError("Binance returned no BNBUSDT 1h klines")
    frame = pd.DataFrame(
        rows,
        columns=[
            "open_time",
...
def write_daily_partitions(
    raw: pd.DataFrame, normalized: pd.DataFrame
) -> dict[str, int]:
...
def enrich_contract_snapshot(engine: Any, *, timeout: float) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Unexpected exchangeInfo payload during enrichment")
    matches = [
    ...
    contract.update(
        {
            "trigger_protect": symbol.get("triggerProtect"),
            "order_types": symbol.get("orderTypes"),
            "time_in_force": symbol.get("timeInForce"),
            "liquidation_fee": symbol.get("liquidationFee"),
            "market_take_bound": symbol.get("marketTakeBound"),
...
def main() -> None:
    ...
    contract = enrich_contract_snapshot(engine, timeout=args.timeout)
    ...
    research_frame_path = ARTIFACT_DIR / "bnb_binance_1h_closed_klines_2y.parquet"
    normalized.to_parquet(research_frame_path, index=False)
    funding.to_csv(
        ARTIFACT_DIR / "bnb_binance_funding_history_2y.csv", index=False
    )
    ...
    metadata = {
        ...
        "paths": {
            ...
            "exact_research_frame": str(research_frame_path.relative_to(ROOT)),
        },
        "funding": {
            ...
            "max_gap_hours": float(funding_gaps.max().total_seconds() / 3600.0),
    ...
    output = ARTIFACT_DIR / "bnb_binance_1h_data_quality_2y.json"
    output.write_text(
