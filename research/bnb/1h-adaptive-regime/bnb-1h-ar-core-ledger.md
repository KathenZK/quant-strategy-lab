...
| `BNB-1H-Adaptive-Regime-V2` | clean-equivalent diagnostic observation；not promoted | V1 clean 参数可执行版本；逐笔重放 trade signature 与 V1 相等（full `133` 笔含 warmup 段）；指标继承 V1：prefit `2.20x / 87.04% / -18.66%`，locked OOS `0.64x / 68.42% / -22.86%`，full `1.87x / 84.25% / -22.86%`；多窗口分片已落盘；V2 全参数消融 `122` rows：`27` 活动字段、`0` 可再删 | `canonical-specs/bnb-1h-ar-v2-parameter-spec-2026-07-07.md`；`research-notes/bnb-1h-ar-v2-multiwindow-backtest-2026-07-07.md`；`ablations/bnb-1h-ar-v2-full-parameter-ablation-2026-07-07.md`；`scripts/bnb_1h_ar_v2.py` | 与 V1 同路径，locked OOS 失败结论继承；不可实盘 |
| `BNB-1H-Adaptive-Regime-V3` | tuned diagnostic observation；not promoted | V2 消融引导微调登记版本；实际最大暴露 `2.5x`；prefit `3.37x / 89.42% / -18.24%`，reused locked OOS `1.22x / 81.25% / -15.53%`，full `2.94x / 88.33% / -18.24%`；主要变更：`ema_pullback` 改 `144` 慢 EMA、启用 trailing exit、杠杆 `2.5x`；`wick_reject` 提高 ADX/量价过滤并使用 `1.0x` | `canonical-specs/bnb-1h-ar-v3-parameter-spec-2026-07-07.md`；`research-notes/bnb-1h-ar-v2-micro-tune-2026-07-07.md` | OOS 属于 reused observation；未经过未读 forward 验证，不可 candidate / paper-live / dry-run / handoff / live |
