...
- `BNB-1H-Adaptive-Regime`（`BNB-1H-AR`）：Binance USD-M Futures `BNBUSDT` perpetual `1h` 多指标自适应 regime 广泛搜索；`BNB-1H-Adaptive-Regime-V1` 已登记为 `ema_pullback+wick_reject` diagnostic observation，并完成全参数消融/clean spec；locked OOS 未通过，当前为 `NO-GO / not promoted / not live-ready`。
