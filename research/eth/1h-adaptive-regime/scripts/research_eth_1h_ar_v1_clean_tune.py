...
RSI_POOL_CSV = ARTIFACT_DIR / f"eth_1h_ar_v1_tune_rsi_pool_{DATE_TAG}.csv"
PAIR_CSV = ARTIFACT_DIR / f"eth_1h_ar_v1_tune_pairs_{DATE_TAG}.csv"
TRADES_CSV = ARTIFACT_DIR / f"eth_1h_ar_v1_tune_selected_trades_{DATE_TAG}.csv"
REPORT_MD = NOTES_DIR / f"eth-1h-ar-v1-clean-parameter-tune-{DATE_TAG}.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prefit-only clean-surface tune for ETH-1H-Adaptive-Regime-V1."
    )
...
def pair_score(
    ...
    high_win_penalty = max(0.0, prefit["win_rate"] - 0.85) * 8.0
    score = (
...
def random_bb_break(rng: random.Random) -> clean.BBBreakCleanConfig:
    return clean.BBBreakCleanConfig(
        ema_htf=rng.choice((55, 89, 144, 233)),
        indicator_window=rng.choice((32, 48, 72, 96)),
        band_k=rng.choice((1.5, 1.75, 2.0, 2.25, 2.5)),
        roc_window=rng.choice((3, 6, 12, 24, 48)),
        min_adx=rng.choice((0.0, 8.0, 12.0, 16.0, 20.0, 24.0, 28.0)),
        min_rvol=rng.choice((0.8, 1.0, 1.5, 2.0, 2.5)),
        ...
        tp_atr=rng.choice((2.0, 2.5, 3.0, 3.5, 4.0, 5.0)),
        sl_atr=rng.choice((1.5, 2.0, 2.5, 3.0, 3.5, 4.0)),
...
def random_rsi(rng: random.Random) -> clean.RSICleanConfig:
    return clean.RSICleanConfig(
        ...
        threshold_low=rng.choice((10.0, 15.0, 20.0, 25.0, 30.0, 35.0)),
        threshold_high=rng.choice((60.0, 65.0, 70.0, 75.0, 80.0, 85.0)),
        roc_window=rng.choice((3, 6, 12, 24, 48)),
        min_adx=rng.choice((0.0, 8.0, 12.0, 16.0, 20.0, 24.0, 28.0)),
        max_adx=rng.choice((30.0, 36.0, 45.0, 55.0, 100.0)),
        min_atr_bps=rng.choice((0.0, 50.0, 75.0, 100.0, 150.0)),
        min_dir_roc_bps=rng.choice((-10000.0, -100.0, 0.0, 50.0, 100.0, 200.0)),
        max_dist_ema_bps=rng.choice((500.0, 750.0, 1000.0, 1500.0, 2500.0, 10000.0)),
        tp_atr=rng.choice((2.0, 2.5, 3.0, 3.5, 4.0, 5.0)),
        sl_atr=rng.choice((1.0, 1.5, 2.0, 2.5, 3.0)),
        max_hold_bars=rng.choice((6, 12, 18, 24, 36, 48)),
        cooldown_bars=rng.choice((0, 3, 6, 12, 24)),
        fixed_leverage=rng.choice((0.5, 1.0, 1.25, 1.5, 2.0, 2.5)),
    )
...
def build_leg_pool(
    *,
    ...
    eligible = 0
    baseline_cfg: Any = (
        clean.BBBreakCleanConfig()
        if component == "bb_break"
        else clean.RSICleanConfig()
    )
    ...
    for index, cfg in enumerate(candidates, start=1):
        ...
        base_cfg = (
            ...
            if component == "bb_break"
            else clean.rsi_to_base(engine, cfg)
        )
...
def simulate_pair_scenario(
    *,
    ...
    try:
        k_cfg = replace(
            clean.bb_break_to_base(engine, bb_break), entry_delay_bars=delay
        )
        c_cfg = replace(clean.rsi_to_base(engine, rsi), entry_delay_bars=delay)
...
def pair_row(candidate: PairCandidate) -> dict[str, Any]:
    row = {
        ...
        **{f"k_{key}": value for key, value in asdict(candidate.bb_break).items()},
        **{f"c_{key}": value for key, value in asdict(candidate.rsi).items()},
...
def main() -> None:
    ...
    bb_break_pool, k_counts = build_leg_pool(
        engine=engine,
        frame=frame,
    ...
    rsi_pool, c_counts = build_leg_pool(
        engine=engine,
        frame=frame,
        funding_times=funding_times,
        funding_cumulative=funding_cumulative,
    ...
    pd.DataFrame(leg_rows(bb_break_pool)).to_csv(BB_BREAK_POOL_CSV, index=False)
    ...
    evaluated_pairs = 0
    eligible_pairs = 0
    for bb_break_item in bb_break_pool:
        for rsi_item in rsi_pool:
            merged = engine.merge_trade_sets(
                bb_break_item.trades,
                rsi_item.trades,
                bb_break_item.score,
                rsi_item.score,
            )
            ...
            pairs = retain_pairs(
                pairs,
                PairCandidate(
                    bb_break=bb_break_item.clean_config,
    ...
    payload = {
        ...
        "observation_id": "ETH-1H-AR-V1-CLEAN-TUNE-2026-07-03",
    ...
    lines = [
        ...
        "",
        (
            "本轮在全参数消融后的 29 个 active clean 参数上做 prefit-only 搜索。"
            f"冻结候选选择规则为 `{selection_reason}`；reused holdout 在冻结后才读取，不参与排序。"
        ),
        "",
        f"- 每腿随机样本：`{args.leg_samples}`；保留 BB breakout/RSI：`{len(bb_break_pool)}` / `{len(rsi_pool)}`。",
    ...
    lines.extend(
        f"- `{key}` = `{value}`" for key, value in asdict(selected.rsi).items()
    )
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            ...
            f"- `artifacts/{BB_BREAK_POOL_CSV.name}`",
