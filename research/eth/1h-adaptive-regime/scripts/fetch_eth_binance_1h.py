...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch and audit the latest two years of closed Binance ETHUSDT perpetual 1h bars."
    )
...
def fetch_two_year_klines(engine: Any, *, timeout: float, cutoff_ms: int) -> pd.DataFrame:
    ...
    while cursor < end_ms:
        ...
        time.sleep(0.05)
    if not rows:
        raise RuntimeError("Binance returned no ETHUSDT 1h klines")
    frame = pd.DataFrame(
        rows,
        columns=[
            "open_time",
...
def main() -> None:
    ...
    normalized.to_parquet(
        ARTIFACT_DIR / "eth_binance_1h_closed_klines_2y.parquet", index=False
    )
    funding.to_csv(
        ARTIFACT_DIR / "eth_binance_funding_history_2y.csv", index=False
    )
    ...
    metadata = {
        ...
        "paths": {
            ...
            "exact_research_frame": str(
                (
                    ARTIFACT_DIR / "eth_binance_1h_closed_klines_2y.parquet"
                ).relative_to(ROOT)
            ),
        },
        "funding": {
    ...
    output = ARTIFACT_DIR / "eth_binance_1h_data_quality_2y.json"
    output.write_text(
