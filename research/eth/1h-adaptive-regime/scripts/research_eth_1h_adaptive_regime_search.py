...
DATA_PATH = ARTIFACT_DIR / "eth_binance_1h_closed_klines_2y.parquet"
QUALITY_PATH = ARTIFACT_DIR / "eth_binance_1h_data_quality_2y.json"
FUNDING_PATH = (
    ROOT
    / "data/normalized/funding/exchange=binance/market_type=perp"
    / "symbol=eth_usdt_usdt/funding.parquet"
)
DATE_TAG = "2026-07-03"
SUMMARY_JSON = ARTIFACT_DIR / f"eth_1h_adaptive_regime_search_{DATE_TAG}.json"
PREFIT_CSV = ARTIFACT_DIR / f"eth_1h_adaptive_regime_prefit_{DATE_TAG}.csv"
RANKING_CSV = ARTIFACT_DIR / f"eth_1h_adaptive_regime_ranking_{DATE_TAG}.csv"
SLICES_CSV = ARTIFACT_DIR / f"eth_1h_adaptive_regime_slices_{DATE_TAG}.csv"
TRADES_CSV = ARTIFACT_DIR / f"eth_1h_adaptive_regime_top_trades_{DATE_TAG}.csv"
REPORT_MD = DIAGNOSTIC_DIR / f"eth-1h-adaptive-regime-search-{DATE_TAG}.md"
...
def load_engine() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("eth_1h_search_engine", ENGINE_PATH)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Broad locked-three-month-OOS ETHUSDT 1h adaptive-regime search."
    )
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    if not DATA_PATH.exists() or not QUALITY_PATH.exists() or not FUNDING_PATH.exists():
        raise FileNotFoundError(
            "Run scripts/fetch_eth_binance_1h.py before the search."
        )
    ...
    if blockers:
        raise RuntimeError(
            f"ETHUSDT 1h exact research frame has data-quality blockers: "
...
def eth_name(name: str) -> str:
    return name.replace("HYPE_1H_AR", "ETH_1H_AR")
...
def make_report(
    ...
    *,
    ...
    lines = [
        ...
        "## 数据质量",
        "",
        f"- Binance USD-M Futures `ETHUSDT` perpetual `1h`：`{quality['rows']}` 根闭合 K。",
...
def main() -> None:
    ...
    configs = [
        replace(cfg, name=eth_name(cfg.name)) for cfg in engine.curated_configs()
    ]
    random_offset = len(configs)
    configs.extend(
        replace(
            engine.random_config(rng, index + random_offset),
            name=f"ETH_1H_AR_R{index + random_offset:06d}",
        )
    ...
    if not retained:
        raise RuntimeError("No eligible ETH configs survived the prefit gates")
