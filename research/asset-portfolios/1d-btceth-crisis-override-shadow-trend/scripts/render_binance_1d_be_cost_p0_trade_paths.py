...
def render(frontier: str, run_date: str) -> Path:
    stem = ARTIFACT_DIR / f"binance_1d_be_cost_p0_{run_date}"
    ...
    trades = trades.loc[(trades["frontier"].eq(frontier)) & (trades["mode"].eq("base"))].copy()
    ...
    for number, trade in enumerate(trade_payload, start=1):
        trade["number"] = number
    payload = {
        "frontier": frontier,
