"""P1 baseline for BIN-1H-EMAX-LGBM: 1h EMA21/96 cross events, raw material check.

Timeframe port of the archived 15m family's P1: extract all dev-window cross
events on closed 1h bars, label the three pre-registered brackets with the
same conservative fill rules and Binance cost model, and report cost/ATR
distribution, label distributions, and gross/net expectations (all + trading
pool + per side + per year + audit-only recent slices).

Development window only: entries whose 96-bar (96h) label window closes
before 2026-01-01 UTC. 2026H1 is a contaminated holdout for this mechanism
(revealed by BIN-15M-EMAX-LGBM and BIN-1H-CSLGBM); a clean OOS for this line
must be prospective (2026-07+).

Reuses pure functions from the frozen 15m engine (emax_common.py, pinned by
SHA256 in the 15m freeze manifest — do not modify it); paths, bar-per-day
coverage (24), and the dev cutoff are adapted here.
"""
...
FAMILY_DIR = ROOT / "research/asset-portfolios/1h-ema-cross-lightgbm-event-selector"
ARTIFACT_DIR = FAMILY_DIR / "artifacts"
CACHE_DIR = ROOT / "data/cache/emax_1h"
...
# last entry whose 96-bar (96h) label window closes strictly before 2026-01-01
DEV_ENTRY_CUTOFF = pd.Timestamp("2026-01-01", tz="UTC") - pd.Timedelta(hours=ec.HORIZON_BARS + 1)
...
def kline_globs() -> list[str]:
    ...
    # (The original comment here claimed date=* was redundant overlap; that was wrong.)
    return [
...
def ensure_symbol_partition_cache() -> Path:
    ...
    con.execute(
        f"""
        COPY (
            SELECT sym_key, ts, open, high, low, close,
                   volume, quote_volume, trade_count,
                   taker_buy_volume, taker_buy_quote_volume
            FROM (
                SELECT
                    {ec.sym_key_expr()} AS sym_key,
...
def load_symbol_frame(sym_key: str) -> pd.DataFrame:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError(f"duplicate 1h timestamps for {sym_key}")
    return frame
...
def build_universe(daily: pd.DataFrame) -> pd.DataFrame:
    """15m engine's point-in-time universe with 1h coverage (24 bars/day)."""
...
def extract_symbol(
    ...
    entry_floor: pd.Timestamp = DEV_START,
    entry_cutoff: pd.Timestamp = DEV_ENTRY_CUTOFF,
) -> pd.DataFrame | None:
...
def main() -> None:
    print("building 1h symbol cache (one-off)...", flush=True)
    ...
    events_path = ARTIFACT_DIR / "events_dev_1h.parquet"
    ...
    report: dict = {
        ...
        "family": "Binance-1H-EMA-Cross-LightGBM-Event-Selector",
        "market": "Binance USD-M USDT perp, 1h, point-in-time universe",
        ...
        "cluster_same_hour_same_side": {
            "p99": float(
                events.groupby([pd.to_datetime(events["entry_ts"], utc=True).dt.floor("h"), "side"])
                .size()
                .quantile(0.99)
            ),
            "max": int(
    ...
    for name in ec.BRACKETS:
        ...
        entry["by_year_net_mean_atr"] = {
            str(year): round(float(group[f"{name}_net_atr"].mean()), 4)
            for year, group in events.groupby(years)
        }
        entry["by_year_gross_mean_atr"] = {
            str(year): round(float(group[f"{name}_gross_atr"].mean()), 4)
            for year, group in events.groupby(years)
        }
    ...
    output = ARTIFACT_DIR / "baseline_1h_report.json"
    output.write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps({k: report[k] for k in ["events", "cost_atr", "cluster_same_hour_same_side"]}, indent=2))
