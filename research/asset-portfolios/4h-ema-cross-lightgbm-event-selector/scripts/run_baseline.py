"""P1 baseline for BIN-4H-EMAX-LGBM: 4h EMA21/96 cross events, raw material check.

Third point of the timeframe-gradient scan (15m -> 1h -> 4h). There is no
full-market 4h archive in the lake, so 4h bars are DERIVED here by
deterministic resampling of the audited 1h Vision archives (UTC 4h-boundary
...
CACHE_DIR = ROOT / "data/cache/emax_4h_derived"
...
DEV_ENTRY_CUTOFF = pd.Timestamp("2026-01-01", tz="UTC") - pd.Timedelta(hours=4 * (ec.HORIZON_BARS + 1))

PROVENANCE = {
    "derived_field": "4h OHLCV bars",
    "derivation": "UTC 4h-boundary aggregation of 1h bars: open=first, high=max, "
    "low=min, close=last, volume/quote_volume/trade_count/taker_*=sum; "
    "partial 4h bars kept with bars_1h count recorded",
    ...
    "null_policy": "no filling; missing 1h bars simply shrink the 4h aggregate",
}
...
def ensure_symbol_partition_cache() -> Path:
    ...
    con.execute(
        ...
        COPY (
            ...
            WHERE rn = 1
            GROUP BY sym_key, time_bucket(INTERVAL '4 hours', ts)
...
def load_symbol_frame(sym_key: str) -> pd.DataFrame:
    ...
    if frame["ts"].duplicated().any():
        raise RuntimeError(f"duplicate 4h timestamps for {sym_key}")
    return frame
...
def build_universe(daily: pd.DataFrame) -> pd.DataFrame:
    """15m engine's point-in-time universe with 4h coverage (6 bars/day)."""
...
def main() -> None:
    print("building derived 4h symbol cache (one-off)...", flush=True)
    ...
    events_path = ARTIFACT_DIR / "events_dev_4h.parquet"
    ...
    report: dict = {
        ...
        "family": "Binance-4H-EMA-Cross-LightGBM-Event-Selector",
        "market": "Binance USD-M USDT perp, 4h (derived from audited 1h), point-in-time universe",
    ...
    output = ARTIFACT_DIR / "baseline_4h_report.json"
