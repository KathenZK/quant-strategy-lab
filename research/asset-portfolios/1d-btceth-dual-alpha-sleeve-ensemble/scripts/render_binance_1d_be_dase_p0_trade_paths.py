...
def render(frontier: str, run_date: str) -> Path:
    stem = ARTIFACT_DIR / f"binance_1d_be_dase_p0_{run_date}"
    paths = pd.read_csv(f"{stem}_paths.csv", parse_dates=["ts"])
    trades = pd.read_csv(f"{stem}_component_trades.csv", parse_dates=["entry_ts", "exit_ts"])
    path = paths.loc[paths["frontier"].eq(frontier)].copy()
    ...
    payload = {
        ...
        "equity": {
            "ts": [value.isoformat() for value in path["ts"]],
            "combined": path["equity"].astype(float).tolist(),
            "cbct": path["cbct_equity"].astype(float).tolist(),
            "rcr": path["rcr_equity"].astype(float).tolist(),
        },
        "trades": trade_payload,
    }
    output = ARTIFACT_DIR / f"binance_1d_be_dase_p0_{frontier}_trade_path_{run_date}.html"
    html = """<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BIN-1D-BE-DASE trade path</title><script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script><style>:root{color-scheme:dark}body{margin:0;background:#0d131a;color:#e8edf2;font:14px/1.5 Inter,-apple-system,sans-serif}header{padding:16px 22px;border-bottom:1px solid #27313d}h1{font-size:19px;margin:0}p{color:#aab7c4;margin:5px 0 0}#chart{width:100%;height:calc(100vh - 85px);min-height:900px}</style></head><body><header><h1 id="title"></h1><p>UTC｜BTC/ETH 1h close、组合与两个独立 sleeve 净值、104 笔 component entry/exit 连线。CBCT 实线，RCR 点线；绿色 long，红色 short。</p></header><div id="chart"></div><script>const p=__PAYLOAD__;document.getElementById('title').textContent=`BIN-1D-BE-DASE · ${p.frontier.replaceAll('_',' ')} · ${p.trade_count} component trades`;const t=[{type:'scattergl',mode:'lines',x:p.btc.ts,y:p.btc.close,xaxis:'x',yaxis:'y',name:'BTCUSDT',line:{color:'#69b3ff',width:1}},{type:'scattergl',mode:'lines',x:p.eth.ts,y:p.eth.close,xaxis:'x2',yaxis:'y2',name:'ETHUSDT',line:{color:'#a98cff',width:1}},{type:'scattergl',mode:'lines',x:p.equity.ts,y:p.equity.combined,xaxis:'x3',yaxis:'y3',name:'Combined',line:{color:'#f6c85f',width:2.4}},{type:'scattergl',mode:'lines',x:p.equity.ts,y:p.equity.cbct,xaxis:'x3',yaxis:'y3',name:'CBCT sleeve',line:{color:'#19a974',width:1}},{type:'scattergl',mode:'lines',x:p.equity.ts,y:p.equity.rcr,xaxis:'x3',yaxis:'y3',name:'RCR sleeve',line:{color:'#e58b2a',width:1}}];for(const q of p.trades){const btc=q.asset==='BTCUSDT';t.push({type:'scatter',mode:'lines+markers',x:[q.entry_ts,q.exit_ts],y:[q.entry,q.exit],xaxis:btc?'x':'x2',yaxis:btc?'y':'y2',showlegend:false,line:{color:q.side>0?'#19a974':'#e45756',width:q.sleeve==='CBCT'?2:1.3,dash:q.sleeve==='CBCT'?'solid':'dot'},marker:{size:5},text:[`#${q.number} ${q.sleeve} ${q.asset} ${q.side>0?'LONG':'SHORT'} entry`,`#${q.number} ${q.sleeve} ${q.reason} exit · log growth ${q.log_growth.toFixed(3)}`],hovertemplate:'%{x}<br>%{y:,.2f}<br>%{text}<extra></extra>'});}const xa={type:'date',gridcolor:'#202a35',rangeslider:{visible:false}},ya={type:'log',gridcolor:'#202a35'};Plotly.newPlot('chart',t,{paper_bgcolor:'#0d131a',plot_bgcolor:'#0d131a',font:{color:'#dce5ed'},margin:{l:65,r:25,t:25,b:50},xaxis:{...xa,domain:[0,1],anchor:'y'},yaxis:{...ya,domain:[.68,1],title:'BTCUSDT'},xaxis2:{...xa,domain:[0,1],anchor:'y2'},yaxis2:{...ya,domain:[.34,.65],title:'ETHUSDT'},xaxis3:{...xa,domain:[0,1],anchor:'y3'},yaxis3:{...ya,domain:[0,.30],title:'equity multiple'}},{responsive:true,displaylogo:false});</script></body></html>"""
    output.write_text(html.replace("__PAYLOAD__", json.dumps(payload, separators=(",", ":"))), encoding="utf-8")
    return output


def main() -> None:
    parser = argparse.ArgumentParser(description="Render DASE component and combined trade paths.")
