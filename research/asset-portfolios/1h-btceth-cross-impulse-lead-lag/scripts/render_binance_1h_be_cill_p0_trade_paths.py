...
def render(frontier: str, run_date: str) -> Path:
    stem = ARTIFACT_DIR / f"binance_1h_be_cill_p0_frontiers_{run_date}"
    ...
    trade_payload = [{
        "number": number + 1, "leader": trade["leader"], "follower": trade["follower"], "side": int(trade["side"]),
        "entry_ts": pd.Timestamp(trade["entry_ts"]).isoformat(), "exit_ts": pd.Timestamp(trade["exit_ts"]).isoformat(),
        "entry": float(trade["entry_fill"]), "exit": float(trade["exit_fill"]), "log_growth": float(trade["trade_log_growth"]),
    } for number, trade in trades.reset_index(drop=True).iterrows()]
    payload = {
        "frontier": frontier, "trade_count": len(trades),
