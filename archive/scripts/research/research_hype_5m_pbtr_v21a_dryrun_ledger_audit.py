...
REPORT_PATH = Path("reports/hype_5m_pbtr_v21a_dryrun_ledger_audit.json")
TRADE_CSV_PATH = Path("reports/hype_5m_pbtr_v21a_dryrun_ledger_audit_trades.csv")
MARKDOWN_PATH = Path(
...
def render_markdown(df: pd.DataFrame, summary: dict[str, Any], events: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## 产物",
            "",
            "- 脚本：`archive/scripts/research/research_hype_5m_pbtr_v21a_dryrun_ledger_audit.py`",
            f"- JSON：`{REPORT_PATH}`",
