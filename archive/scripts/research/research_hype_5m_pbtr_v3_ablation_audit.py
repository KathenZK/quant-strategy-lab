...
REPORT_PATH = Path("reports/hype_5m_pbtr_v3_ablation_audit.json")
ABLATION_SUMMARY_PATH = Path("reports/hype_5m_pbtr_v3_ablation_summary.csv")
ABLATION_SLICES_PATH = Path("reports/hype_5m_pbtr_v3_ablation_validation_slices.csv")
WEEKLY_PATH = Path("reports/hype_5m_pbtr_v3_weekly_slices.csv")
MONTHLY_PATH = Path("reports/hype_5m_pbtr_v3_monthly_slices.csv")
ROLLING_PATH = Path("reports/hype_5m_pbtr_v3_rolling_windows.csv")
AUDIT_PATH = Path("reports/hype_5m_pbtr_v3_audit_metrics.json")
MARKDOWN_PATH = Path("docs/research/hype/families/5m-pullback-trail/diagnostics/hype-5m-pbtr-v3-ablation-audit-2026-06-24.md")
...
def render_markdown(summary: pd.DataFrame, weekly: pd.DataFrame, monthly: pd.DataFrame, rolling: pd.DataFrame, audit: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## 产物",
            "",
            f"- 脚本：`archive/scripts/research/research_hype_5m_pbtr_v3_ablation_audit.py`",
            f"- JSON：`{REPORT_PATH}`",
