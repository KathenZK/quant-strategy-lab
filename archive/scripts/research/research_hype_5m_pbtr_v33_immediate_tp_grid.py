...
REPORT_PATH = Path("reports/hype_5m_pbtr_v33_immediate_tp_grid.json")
GRID_PATH = Path("reports/hype_5m_pbtr_v33_immediate_tp_grid.csv")
MARKDOWN_PATH = Path(
...
def render_markdown(grid: pd.DataFrame) -> str:
    ...
    lines.extend(
        [
            ...
            "## 产物",
            "",
            "- 脚本：`archive/scripts/research/research_hype_5m_pbtr_v33_immediate_tp_grid.py`",
            f"- JSON：`{REPORT_PATH}`",
            f"- 网格 CSV：`{GRID_PATH}`",
        ]
    )
