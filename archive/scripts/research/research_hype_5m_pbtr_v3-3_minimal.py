...
REPORT_PATH = Path("reports/hype_5m_pbtr_v3-3_minimal.json")
SUMMARY_PATH = Path("reports/hype_5m_pbtr_v3-3_minimal_summary.csv")
ROLLING_PATH = Path("reports/hype_5m_pbtr_v3-3_minimal_rolling.csv")
WEEKLY_PATH = Path("reports/hype_5m_pbtr_v3-3_minimal_weekly.csv")
MONTHLY_PATH = Path("reports/hype_5m_pbtr_v3-3_minimal_monthly.csv")
TRADES_PATH = Path("reports/hype_5m_pbtr_v3-3_minimal_trades.csv")
MARKDOWN_PATH = Path("docs/research/hype/families/5m-pullback-trail/diagnostics/hype-5m-pbtr-v3-3-minimal-2026-06-24.md")
...
def render_markdown(summary: pd.DataFrame, rolling: pd.DataFrame, weekly: pd.DataFrame, monthly: pd.DataFrame) -> str:
    ...
    lines.extend(
        [
            ...
            "## 产物",
            "",
            f"- 脚本：`archive/scripts/research/research_hype_5m_pbtr_v3-3_minimal.py`",
            f"- JSON：`{REPORT_PATH}`",
