...
from strategy_lab.journal.sqlite_store import RunSqliteStore


@dataclass(frozen=True, slots=True)
class BacktestJournalEntry:
    kind: str
    name: str
    run_id: str
    generated_at: str
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> "BacktestJournalEntry":
        return cls(
...
class BacktestJournal:
    ...
    def append(self, entry: BacktestJournalEntry, *, manifest_payload: dict[str, Any] | None = None) -> Path:
    ...
    def backfill_from_jsonl(self) -> dict[str, Any]:
        ...
        for row in self._load_jsonl_rows():
            summary["processed"] += 1
            entry = BacktestJournalEntry.from_dict(row)
...
RunRegistryEntry = BacktestJournalEntry
RunRegistry = BacktestJournal
