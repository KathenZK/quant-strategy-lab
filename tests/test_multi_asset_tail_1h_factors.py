    ...
    return pd.DataFrame(
        {
            ...
            * (0.5 + 0.1 * np.sin(index / 9.0)),
            ...
            "funding_event_rate": np.where(index % 8 == 0, 0.0001, 0.0),
        }
    )


def test_tail_registry_is_large_unique_and_contains_risk_features() -> None:
    registry = multi_asset_tail_1h_registry()

    assert len(registry.names()) == 182
    assert len(registry.names()) == len(set(registry.names()))
    assert "upside_vol_168" in registry.names()
    assert "jump_count_up_3pct_72" in registry.names()
    assert "funding_event_sum_168" in registry.names()
    assert "mark_premium_max_72" in registry.names()


def test_tail_factors_do_not_read_future_rows() -> None:
    registry = multi_asset_tail_1h_registry()
    ...
    for name in registry.names():
        left = registry.get(name).compute(original)
        right = registry.get(name).compute(perturbed)
